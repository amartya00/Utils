...
class TimeUtils:
	...
	for f in TimeUtils.validformats:
		...
		return retval
	raise TimeUtilsException("Invalid time format: " + datestr + ". Allowed formats are: " + str(TimeUtils.validformats) + " and " + str(TimeUtils.validRelativeTimes))
	...
	parser = argparse.ArgumentParser(prog = "TimeUtils", description = __doc__, usage = "Time [options]", formatter_class = argparse.RawTextHelpFormatter)
