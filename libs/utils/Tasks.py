...
class Tasks:
    ...
    def tickChecklist(self, itemNum):
        ...
        try:
            ...
            raise TasksException(str(
                itemNum) + " is not a valid integer. This argument needs to be an integer (the index of the checklist item)")
        if "Checklist" not in self.config.keys():
            self.config["Checklist"] = []
    ...
    def archiveTask(self):
        ...
        if self.config["STATUS"].lower() == "complete":
            ...
            shutil.move(os.path.join(self.taskRoot + "/../", self.archiveFile),
                        Tasks.archiveRoot + "/" + self.archiveFile)
    ...
    def refreshTasksData():
        open(Tasks.tasksData, "w").write(json.dumps(
            [Tasks(t).config for t in os.listdir(Tasks.tasksRoot) if os.path.isdir(os.path.join(Tasks.tasksRoot, t))],
            indent=4))
    ...
    def getOpts(cmdLineArgs):
        parser = argparse.ArgumentParser(prog="Tasks", description=__doc__, usage="Tasks [options]",
        ...
        parser.add_argument("-t", "--tickChecklist",
                            help="Tick a checklist item. Provide the integer index of the item in the list.")
        parser.add_argument("-c", "--clean", help="Clean all tasks with status of 'Complete' (case ignored)",
                            action="store_true")
