...
from libs.utils.ToDoItem import TodoException
...
class Todo:
    ...
    def downloadAndMerge(self):
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "404":
                raise TodoException("Either bucket (" + self.conf["Bucket"] + ") or key (" + self.conf[
                    "Key"] + ") not found. If you never uploaded the todos, ignore this")
            else:
    ...
    def writeBack(self):
        open(os.path.join(Todo.root, self.conf["Key"]), "w").write(
            json.dumps({"Todos": self.todos["Todos"].toMap()}, indent=4))
        return self
    ...
    def getOpts(cmdLineArgs):
        parser = argparse.ArgumentParser(prog="Todo", description=__doc__, usage="Todo [options]",
        ...
        parser.add_argument("-u", "--update",
                            help="Update due date or status for a to do item. This has to be followed by the -i option and the -d / -t option.",
                            action="store_true")
        parser.add_argument("-x", "--delete", help="Delete a to do item. This has to be followed by the -i option.",
                            action="store_true")
