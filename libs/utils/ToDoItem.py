...
class TodoItem:
	...
	try:
		...
		self.data["CreateDate"] = TimeUtils.getDate(attributes["CreateDate"], False).strftime(TimeUtils.validformats[0])

	    if "DueDate" in attributes.keys() and not attributes["DueDate"] == None:
		self.data["DueDate"] =  TimeUtils.getDate(attributes["DueDate"], False).strftime(TimeUtils.validformats[0])

	    if "LastModified" in attributes.keys() and not attributes["LastModified"] == None:
		self.data["LastModified"] = TimeUtils.getDate(attributes["LastModified"], False).strftime(TimeUtils.validformats[0])

	    if "Status" in attributes.keys() and not attributes["Status"] == None:
		if attributes["Status"].upper() not in TodoItem.validStatus:
		    raise TodoException("Invalid todo status: '" + attributes["Status"] + "'. valid statuses are: " + str(TodoItem.validStatus))
