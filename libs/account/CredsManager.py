...
class CredsManager:
    ...
    def configure(self):
        ...
        if profileName == "":
            profileName = "DEFAULT"
        profile = self.conf["Creds"].data[profileName] if profileName in self.conf["Creds"].data.keys() else Creds(
            profileName, "CUSTOM")
        profile.data["Name"] = profileName

        promptAccessKey = " [] " if profile.data["AccessKey"] == None else (
        " [" + profile.data["AccessKey"][0:3] + "***] ")
        promptSecretKey = " [] " if profile.data["SecretKey"] == None else (
        " [" + profile.data["SecretKey"][0:3] + "***] ")
        promptSessionToken = " [] " if profile.data["SessionToken"] == None else (
        " [" + profile.data["SessionToken"][0:3] + "***] ")
    ...
    def getOpts(cmdLineArgs):
        parser = argparse.ArgumentParser(prog="CredsManager", description=__doc__, usage="CredsManager [options]",
